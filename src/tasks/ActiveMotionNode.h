 ...
 */
template <typename CmdType, typename TelType> class ActiveMotionNode {
protected:
...
public:
  ActiveMotionNode()
      : cmdQueue(nullptr), telQueue(nullptr), taskHandle(nullptr),
        taskName(nullptr), priority(0), stackDepth(0), lastWakeTime(0) {}
  ...
  /**
   * @brief Main task loop - drains commands, updates hardware, publishes
   * telemetry.
   */
  void taskLoop() {
    hwInit();
  ...
  bool start(const char *name, uint32_t stackDepth, UBaseType_t priority) {
    ...
    BaseType_t result = xTaskCreate(
        ...
        name, stackDepth, this, priority, &taskHandle);
